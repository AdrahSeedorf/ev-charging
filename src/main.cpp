...
int cmdInspect(const Options& options) {
    ...
    for (const auto& node : network.nodes()) {
        ...
        if (node.hasStation()) {
            ...
            price << std::fixed << std::setprecision(2) << node.station->pricePerUnit;
            std::cout << std::setw(12) << (node.station->pricePerUnit == 0.0 ? "free" : price.str())
                      << std::setw(11) << node.station->servers << std::setw(9)
                      << static_cast<int>(node.station->ratePerHour);
        } else {
...
void writeTimeSeries(const std::string& path,
    ...
    for (const auto& sample : sampleTimeSeries(network, runtime, horizon, interval)) {
        const double used = sample.servers > 0
                                ? static_cast<double>(sample.charging) / sample.servers
                                : 0.0;
        out << std::fixed << std::setprecision(3) << sample.time << ","
            << sample.node << "," << csv::escape(sample.station) << "," << sample.waiting << ","
            << sample.charging << "," << sample.servers << ","
...
int cmdSimulateEvents(const Options& options) {
    ...
    for (const NodeId id : network.stationNodes()) {
        ...
        std::cout << std::left << std::setw(18) << fit(network.node(id).name, 18) << std::right
                  << std::setw(10) << network.node(id).station->servers << std::setw(10)
...
int cmdSimulateStatic(const Options& options) {
    ...
    for (const NodeId id : network.stationNodes()) {
        ...
        std::cout << std::left << std::setw(18) << fit(network.node(id).name, 18) << std::right << std::setw(10)
                  << network.node(id).station->servers << std::setw(9) << queue << std::setw(14)
...
Station sitePrototype(const Options& options) {
    Station prototype;
    prototype.pricePerUnit = options.newStationPrice;
    prototype.servers = options.newStationChargers;
    prototype.ratePerHour = options.newStationPower;
    return prototype;
}
